...
pub fn initializePaging() EarlyPagingError!void {
    ...
    initializeBootstrapMappings(kernel_page_directory_entries, direct_map_entries, reserved_map);
...
fn flagsForPhysicalPage(physical_address: usize, reserved_map: *const arch.ReservedMap) PageEntryFlags {
    ...
    for (reserved_map.entries[0..reserved_map.length]) |reservation| {
        ...
        if (physical_address < reservation_end and physical_page_end > reservation.address) {
            ...
            flags.writeable = flags.writeable or reservation_flags.writeable;
            flags.user_accessible = flags.user_accessible or reservation_flags.user_accessible;
            flags.global = flags.global or reservation_flags.global;
            flags.cache_disabled = flags.cache_disabled or reservation_flags.cache_disabled;
        }
    }
